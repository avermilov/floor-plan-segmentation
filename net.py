...
class DFPmodel(torch.nn.Module):
    def __init__(
        ...
        rblist = [512, 256, 128, 64, 32, boundary_channels]
        ...
        self.rtgrs = nn.ModuleList(
            [self._conv2d(rtlist[i], rtlist[i], 3, 1, 1) for i in range(1, len(rtlist))]
        )

        # Attention Non-local context
        clist = [256, 128, 64, 32]
        self.ac1s = nn.ModuleList(
            self._conv2d(clist[i], clist[i], 3, 1, 1) for i in range(len(clist))
        )
        self.ac2s = nn.ModuleList(
            self._conv2d(clist[i], clist[i], 3, 1, 1) for i in range(len(clist))
        )
        self.ac3s = nn.ModuleList(
            self._conv2d(clist[i], 1, 1, 1) for i in range(len(clist))
        )
        self.xc1s = nn.ModuleList(
            self._conv2d(clist[i], clist[i], 3, 1, 1) for i in range(len(clist))
        )
        self.xc2s = nn.ModuleList(
            self._conv2d(clist[i], 1, 1, 1) for i in range(len(clist))
        )
        self.ecs = nn.ModuleList(
            self._conv2d(1, clist[i], 1, 1) for i in range(len(clist))
        )
        self.rcs = nn.ModuleList(
            self._conv2d(2 * clist[i], clist[i], 1, 1) for i in range(len(clist))
        )
        ...
        self.ds = nn.ModuleList(
            self._dirawareLayer([1, 1, dim, dim], diag=True) for dim in dak
        )
        # diagonal flip
        self.dfs = nn.ModuleList(
            self._dirawareLayer([1, 1, dim, dim], diag=True, flip=True) for dim in dak
        )
        # Last layer
        self.last = self._conv2d(clist[-1], room_channels, 1, 1)
    ...
    def context_conv2d(
        self, t, dim=1, size=7, diag=False, flip=False, stride=1, trainable=False
    ):
        C = t.size(1)
        in_dim = C
    ...
    def forward(self, x):
        ...
        for i, rbtran in enumerate(self.rbtrans):
            ...
            rbfeatures.append(x)
        logits_cw = F.interpolate(self.rbconvs[-1](x), self.interpolation_size)
        ...
        logits_r = F.interpolate(self.last(x), self.interpolation_size)
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        model = DFPmodel(room_channels=9, boundary_channels=3)
